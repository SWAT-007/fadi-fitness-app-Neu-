...
const emptyForm = (): FoodForm => ({
  name: '', category: 'protein', kcal: '0', protein: '0', carbs: '0', fat: '0',
})
...
export default function FoodsDatabasePage() {
  ...
  const openEdit = (food: Food) => {
    ...
    setForm({
      ...
      kcal: String(food.kcal_per_100g), protein: String(food.protein_per_100g),
      carbs: String(food.carbs_per_100g), fat: String(food.fat_per_100g),
    })
    setError(null)
    setShowForm(true)
  }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      kcal_per_100g: Number(form.kcal) || 0,
    ...
    if (editFood) {
      const { error: err } = await supabase.from('foods').update(payload).eq('id', editFood.id)
      if (err) { setError(err.message); setSaving(false); return }
    } else {
      const { error: err } = await supabase.from('foods').insert({ ...payload, created_by: userId })
      if (err) { setError(err.message); setSaving(false); return }
    }
  ...
  const handleDelete = async (food: Food) => {
    ...
    if (!food.created_by) { alert('System-Lebensmittel können nicht gelöscht werden.'); return }
  ...
  if (loading) {
    return <div className="p-8 flex justify-center"><div className="w-8 h-8 border-4 border-green-500 border-t-transparent rounded-full animate-spin" /></div>
  }
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{foods.length} Lebensmittel · Nährwerte pro 100 g</p>
        </div>
        <button onClick={openAdd} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value as FoodCategory }))}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent">
                  {CATEGORIES.map(c => <option key={c} value={c}>{FOOD_CATEGORY_LABEL[c]}</option>)}
                </select>
              </div>
            </div>

            <div>
              <p className="text-xs font-medium text-gray-600 mb-2">Nährwerte pro 100 g</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input type="number" min={0} step={0.1} value={form[f.key]}
              ...
              {(Number(form.protein) || Number(form.carbs) || Number(form.fat)) > 0 && (
                <p className={`text-xs mt-2 ${Math.abs(macroKcal - Number(form.kcal)) > 30 ? 'text-amber-600' : 'text-green-600'}`}>
                  ...
                  {Math.abs(macroKcal - Number(form.kcal)) > 30 && (
                    <button type="button" className="ml-2 underline" onClick={() => setForm(p => ({ ...p, kcal: String(macroKcal) }))}>
                      Übernehmen
                    </button>
                  )}
                </p>
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={saving}
                ...
                {saving ? 'Speichern…' : editFood ? 'Aktualisieren' : 'Hinzufügen'}
              </button>
            </div>
          </form>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        />
        <select value={filterCat} onChange={e => setFilterCat(e.target.value as FoodCategory | 'all')}
