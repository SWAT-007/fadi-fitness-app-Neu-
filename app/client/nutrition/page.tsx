...
export default function ClientNutritionPage() {
  ...
  const calcMeal = async (mealId: string) => {
    ...
    const { grams } = solveGrams(meal, macroFoods)
    const vegGrams = Math.max(0, Math.round(meal.target_vegetable_g ?? 0))
  ...
  const dayTotals = useMemo(() => sumMacros(cmf), [cmf])
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-4">
      ...
      <div className="space-y-3">
        {plan.nutrition_meals.map((meal, i) => {
          ...
          const allowedRaw = (meal.allowed_categories ?? ['protein', 'carbs', 'fat']) as FoodCategory[]
          ...
          const macroLines = [
            { cat: 'protein' as FoodCategory, cur: t.p, tgt: meal.target_protein },
            { cat: 'carbs'   as FoodCategory, cur: t.k, tgt: meal.target_carbs   },
            { cat: 'fat'     as FoodCategory, cur: t.f, tgt: meal.target_fat     },
          ...
          const allCalculated = items.length > 0 && items.every(it => it.amount_g > 0)
            ...
            <div key={meal.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="px-5 pt-4 pb-3 border-b border-gray-50">
                <div className="flex items-baseline justify-between">
                  ...
                  <span className="text-xs text-gray-500">
                    <b className="text-gray-900">{Math.round(t.cal)}</b> / {targetKcal} kcal
                  </span>
                </div>
              ...
              <div className="divide-y divide-gray-50">
                {allowed.map(cat => {
                  ...
                  const m = calculated ? macrosFor(picked) : null
                  return (
      ...
      <p className="text-[11px] text-center text-gray-400 px-4 pt-2 pb-6">
