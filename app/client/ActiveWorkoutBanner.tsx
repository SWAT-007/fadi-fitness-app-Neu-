...
export default function ActiveWorkoutBanner() {
  ...
  return (
    <Link
      ...
      className="flex items-center justify-between bg-[#A78BFA] hover:bg-[#B79FFB] text-[#050504] px-4 py-3 transition-colors"
    >
      <div className="flex items-center gap-3 min-w-0">
        <span className="relative flex-shrink-0">
          <span className="w-2.5 h-2.5 rounded-full bg-[#050504]/40 block animate-pulse" />
        </span>
        <div className="min-w-0">
          <p className="text-[11px] text-[#050504]/60 font-semibold uppercase tracking-widest leading-none mb-0.5">
            Aktives Training
          </p>
          <p className="font-bold text-sm truncate text-[#050504]">
            {activeWorkout.day?.name ?? 'Training'}
            <span className="font-normal text-[#050504]/60 ml-2 tabular-nums">{formatTime(elapsed)}</span>
          </p>
        </div>
      </div>
      <div className="flex items-center gap-1 bg-[#050504]/15 px-3 py-1.5 rounded-lg flex-shrink-0 ml-3 text-sm font-semibold text-[#050504]">
        Fortsetzen
